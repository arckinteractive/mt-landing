...
class JSON_API {
  ...
  function admin_options() {
    ...
    if (!empty($_REQUEST['_wpnonce']) && wp_verify_nonce($_REQUEST['_wpnonce'], "update-options")) {
      ...
      if (isset($_REQUEST['json_api_woo_ip'])) {
        $this->save_option('json_api_woo_ip', $_REQUEST['json_api_woo_ip']);
      }
      if (isset($_REQUEST['json_api_woo_apikey'])) {
        $this->save_option('json_api_woo_apikey', $_REQUEST['json_api_woo_apikey']);
      }
    }
...
<div class="wrap">
  ...
  <form action="options-general.php?page=json-api" method="post">
    ...
    <?php } ?>
    <h3>Security</h3>
    <p>Specify remote host IP's allowed to access the woocommerce controller and set an API key.</p>
    <table class="form-table">
      <tr valign="top">
        <th scope="row">IP Adresses (comma delimited)</th>
        <td><input type="text" name="json_api_woo_ip" value="<?php echo get_option('json_api_woo_ip'); ?>" size="100" /></td>
      </tr>
    </table>
    <table class="form-table">
      <tr valign="top">
        <th scope="row">API Key</th>
        <td><input type="text" name="json_api_woo_apikey" value="<?php echo get_option('json_api_woo_apikey', sha1(rand())); ?>" size="100" /></td>
      </tr>
    </table>
    <p class="submit">
